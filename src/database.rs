use diesel::{Connection, RunQueryDsl, pg::PgConnection, r2d2, sql_types::Bool};
use std::env;
...
pub async fn create_database_if_not_exists() {
  let db_user = env::var("DB_USERNAME")
    .expect("Environment variable DB_USERNAME must be set");
  let db_pass = env::var("DB_PASSWORD")
    .expect("Environment variable DB_PASSWORD must be set");
  let db_host = env::var("DB_HOST")
    .expect("Environment variable DB_HOST must be set");
  let db_port = env::var("DB_PORT")
    .expect("Environment variable DB_PORT must be set");
  let db_name = env::var("DB_NAME")
    .expect("Environment variable DB_NAME must be set");

  let db_url = format!("postgres://{}:{}@{}:{}/postgres", db_user, db_pass, db_host, db_port);
  let conn = PgConnection::establish(&db_url)
    .expect("Could not connect to PostgreSQL database");

  let query = format!("SELECT EXISTS(SELECT datname FROM pg_catalog.pg_database WHERE datname = '{}') AS exists", db_name);
  let result = diesel::sql_query(query).get_result::<DbExistResult>(&conn)
